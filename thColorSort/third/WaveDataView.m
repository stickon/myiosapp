...
-(void)displayView{
    if (self.waveDataType<=7 || self.waveDataType == wave_rgb_restrain || self.waveDataType == wave_1R_light) {
...
- (void)drawRect:(CGRect)rect withCGContextRef:(CGContextRef)contextRef compressNum:(float)compress color:(UIColor*)uiColor waveIndex:(Byte)index{
    ...
    NSLog(@"wave data:%d",index);
    for (int i = 0; i < self.currentShowWaveLength; i++)
    {
        CGFloat x = i/compress+SPACESIDE;
        float average;
        if (self.waveDataType<=wave_diff_IR || self.waveDataType == wave_rgb_restrain ||self.waveDataType == wave_1R_light) {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.waveDataType == wave_rgb || self.waveDataType == wave_1R_light) {//rgb
    ...
    else if(self.waveDataType == wave_diff)//diff
    {
        ...
        [self drawRect:rect withCGContextRef:currentContext compressNum:compressNum color:[self getDrawColor:_colorType] waveIndex:1];
