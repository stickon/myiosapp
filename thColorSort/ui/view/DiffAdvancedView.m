...
-(void)cellValueChangedWithSection:(long)section row:(long)row tag:(long)index value:(NSInteger)value
{
    ...
    if (section == 0)
    {
        ...
        {
            [[NetworkFactory sharedNetWork]sendAlgorithmSenseValueWithAjustType:index Sorter:row data:value algorithmType:_type FirstSecond:currentSelectFrtsnd ValueType:currentSelectColordiffType IsIR:0];
