...
@interface Rice20CalibrationView()<MyTextFieldDelegate,TipsViewResultDelegate>
{
...
@implementation Rice20CalibrationView

-(instancetype)init{
    self=[super init];
    if(self){
        UIView *subView=[[[NSBundle mainBundle] loadNibNamed:@"Rice20CalibrationView" owner:self options:nil] firstObject];
        [self initView];
...
-(UIView*)getViewWithPara:(NSDictionary *)para{
    ...
    [gNetwork setCameraCalibrationEnable:1];
    [[NetworkFactory sharedNetWork] changeWaveDataWithType:0];
    [gNetwork changeViewPagesWithPageId:0 State:1];
    [self startGetWave];
...
-(BOOL)Back{
    ...
    [gNetwork setCameraCalibrationEnable:0];
    [[NetworkFactory sharedNetWork] changeWaveDataWithType:0];
    [gNetwork changeViewPagesWithPageId:0 State:0];
    [self stopTimer];
    return [super Back];
}
