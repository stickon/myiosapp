...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"%@",[dict valueForKey:@"expertMode"]);
    if ([[dict valueForKey:@"expertMode"] isEqualToString:@"1"]) {
        cell.imageView.image = [UIImage imageNamed:@"icon_expert"];
...
-(void)cellValueChangedWithSection:(long)section row:(long)row tag:(long)index value:(NSInteger)value{
    if (index == 1) {
        ...
        if (device->screenProtocolType == 1 || device->screenProtocolType == 2) {
            [para setObject:[dict valueForKey:@"expertMode"] forKey:@"expertMode"];
        }
