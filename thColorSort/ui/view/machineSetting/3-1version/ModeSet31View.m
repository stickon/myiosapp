...
//  thColorSort
//
//  Created by huanghu on 2018/1/16.
...
-(UIView*)getViewWithPara:(NSDictionary *)para{
    if (para) {
        ...
        NSString *expertMode = [para valueForKey:@"expertMode"];
        self.isExpertMode = expertMode.intValue;
    }
    [self initView];
    [self initLanguage];
    return self;
...
-(void)cellValueChangedWithSection:(long)section row:(long)row tag:(long)index value:(NSInteger)value{//使能
    ...
    if (section == 0) {
        ...
        }else if(row == 2)
        {
            ...
            tempMode[layerIndex].UseRiceReverse = value;
        }else if (row == 18){
            tempMode[layerIndex].UseRiceWhite = value;
        }
    }else if (section == 2){
        if (row == 0) {
            device.currentViewIndex = value;
...
-(BOOL)modeIsChanged{
    ...
    for (int layer = 0; layer<device->machineData.layerNumber; layer++) {
        ...
        if (mode[layer].UseRiceReverse != tempMode[layer].UseRiceReverse) {
            return true;
        }
        if (mode[layer].UseRiceWhite != tempMode[layer].UseRiceWhite) {
            return true;
        }
...
-(void)tipsViewResult:(Byte)value{
    if (value) {
        Device *device = [DataModel globalDataModel].currentDevice;
        NSInteger modeLength = (device->machineData.layerNumber)*sizeof(Mode);
        [[NetworkFactory sharedNetWork] changeModeSettingWithMode:(Byte*)tempMode shape:self.tempShapeIndex svm:self.tempUseSvm hsv:self.tempUseHsv ModeLength:modeLength];
    }else{
         [[NetworkFactory sharedNetWork]getModeSettingWithIndex:self.modeIndex BigModeIndex:self.bigModeIndex];
