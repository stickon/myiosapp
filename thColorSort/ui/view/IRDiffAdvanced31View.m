...
//  thColorSort
//
//  Created by huanghu on 2018/1/12.
...
-(instancetype)init{
    if (self = [super init]) {
        UIView *subView = [[[NSBundle mainBundle]loadNibNamed:@"IRDiffAdvanced31View" owner:self options:nil] firstObject];
        [self initView];
...
-(UIView*)getViewWithPara:(NSDictionary *)para{
    if (para) {
        ...
        self.type = (*(device->irAlgorithm+self.algorithmIndex)).type;
    }
...
-(void)updateWithHeader:(NSData *)headerData{
    ...
    if (header[0] == 0x04 && header[1] == 0x13) {
        dataLoaded = true;
    ...
    if (header[0] == 0x04 && header[1] == 0x14) {
        (*(kDataModel.currentDevice->irAlgorithm+(int)_algorithmIndex)).used = header[4];
    }
