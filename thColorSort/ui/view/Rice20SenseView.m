...
//  thColorSort
//
//  Created by huanghu on 2018/2/6.
...
-(void)updateWithHeader:(NSData *)headerData{
    ...
    }else if (a[0] == 0x04 && (a[1] == 0x22 || a[1] == 0x24)){
        [self refreshCurrentView];
    }else if (a[0] == 0x55){
...
-(UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
    ...
    if (device->machineData.useColor) {
        if (indexPath.row<device.userAlgorithmNums) {
            ...
            memcpy(cell->svmRange, (*(device->riceUserAlgorithm+indexPath.row)).range, 4);
            ...
            for (int i = 0; i < device->groupNum; i++) {
                ...
                [cell.groupTextFieldArray[i] displayView];
            }
