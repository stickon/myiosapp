...
-(void)sendToChangeRiceEnineerSenseSizeWithAlgorithmType:(Byte)algorithmType Group:(Byte)group Type:(Byte)type Value:(NSInteger)value{
    Device *device = kDataModel.currentDevice;
    SenseGroupSetSize sense;
    memset(&sense, 0, sizeof(SenseGroupSetSize));
    ...
    sense.data[0] = value/256;
    sense.data[1] = value%256;
    sense.fSameR = device->riceGroupSense.fSameR;
    [self initSocketHeader];
    socketHeader.type = 0x04;
    socketHeader.extendType = 0x25;
   
    memcpy(socketHeader.data1, (const void*)&sense,sizeof(SenseGroupSetSize));
    [self netWorkSendData];
}
...
-(void)getSvmPara{
    ...
    if (device->screenProtocolMainVersion == 3 ||device->screenProtocolType == 1) {
        socketHeader.extendType = 0x11;
        if (device->machineData.layerNumber == 1&&device.svmGroupNum>1) {
...
-(void)receiveSocketData:(NSData *)data fromAddressIP:(NSString *)IPaddress
{
    ...
    if ([data length] > 0) {
        ...
        [gMiddeUiManager updateCurrentViewWithHeader:data];
         });
    }
    else
    {
